...
function createDeck(): TileInstance[] {
...
function deal5(deck: TileInstance[]) {
...
export default function PlayScreen() {
    ...
    const calculateRoundScore = (currentHands: TileInstance[][]) => {
        ...
        for (const move of roundMoves) {
            ...
            const isSecret = tiles.length === 2 && !isActualPair; // Нууц мод
            ...
            if (!isSecret && referenceColor) {
                if (tileColor && tileColor !== referenceColor) {
                    isValid = false;
                    console.log(`❌ Тоглогч ${move.playerIndex}: Өнгө таарахгүй (${tileColor} vs ${referenceColor})`);
                }
            }
